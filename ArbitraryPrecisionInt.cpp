...
APint& APint::operator=(APint &givenInt) {
    ...
    }else if(givenInt.sizeInBytes > this->sizeInBytes){//if the new integer is larger, then set our precision to match it
...
APint operator+(const APint & left, const APint & right) {
    ...
    for(APint::precisionType i=0; i<left.sizeInBytes; ++i){//special case for the last value which holds sign bit
        if(APIntValMax-(left.value[i]+carry)>right.value[i]){//if max value is lower than the total value, but without writing the max value directly
